...
const REGISTRY_INDEX_WHITELIST: z.infer<typeof registryItemTypeSchema>[] = ["registry:ui", "registry:lib", "registry:hook", "registry:theme", "registry:block"];
...
//function to generate __registry__/index.tsx.
...
`;
  ...
  /*
   * @NOTE :
   * Styles array: An array containing information about various styles or themes to be included in the registry.
   * Loop through each style to generate the appropriate registry entry for each component.
   * * The function iterates through a styles array, creating entries for each style in the Index object.
   */
  ...
  for (const style of styles) {
    ...
    /**
     * @NOTE:
     * INTINYA UNTUK GENERATE PATH DARI SETIP ITEM YG ADA DI FOLDER REGISTRY/
     * This code processes each item in the registry.
     * Each item has a `files` property containing a list of files associated with the component or module.
     * The file paths are converted to valid import paths for use within the registry.
     */
      ...
      if (item.type === "registry:block") {
        ...
        chunks = await Promise.all(
          components.map(async (component, index) => {
            ...
            const containerClassName = containerAttr?.getInitializer()?.asKindOrThrow(SyntaxKind.StringLiteral).getLiteralValue();
      ...
      if (item.files) {
        files = await Promise.all(
          item.files.map(async (_file) => {
            ...
            const content = await fs.readFile(path.join(process.cwd(), "registry", style.name, file.path), "utf8");
