...
function AutoTransitionImageComparison({
  leftSrc,
  rightSrc,
  alt,
  className,
  transitionDuration = 3000,
}: AutoTransitionImageComparisonProps) {
    ...
    <div className={cn(" relative aspect-video w-full", className)}>
      <ImageComparison enableHover={false}>
        <ImageComparisonImage
          position={isLeftVisible ? "left" : "right"}
          src={leftSrc}
          alt={`${alt} - first image`}
        />
        <ImageComparisonImage
          position={isLeftVisible ? "right" : "left"}
          src={rightSrc}
...
function ImageComparisonMarquee({
  comparisons,
  className,
  itemClassName,
  pauseOnHover = false,
  ...
  return (
    ...
    >
      {comparisons.map((item, index) => (
        <AutoTransitionImageComparison
          key={index}
          leftSrc={item.leftSrc}
